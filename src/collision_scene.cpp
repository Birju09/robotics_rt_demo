...
CollisionScene::CollisionScene(const RobotModel& model) : model_(model) {
    ...
    for (const auto& link_mesh : model.linkMeshes()) {
        ...
        if (!bvh) {
            std::cerr << "Warning: Failed to load mesh for link " << link_mesh.link_name
                      << " from " << link_mesh.mesh_path << std::endl;
            continue;
        }
        ...
        lco.bvh_model = bvh;
        lco.local_transform = coal::Transform3s(coal::Matrix3s::Identity(),
    ...
    std::cout << "CollisionScene initialized with " << link_objects_.size() << " collision objects"
              << std::endl;
}

bool CollisionScene::isCollisionFree(const std::vector<double>& q) const {
    if (q.size() != static_cast<size_t>(model_.numJoints())) {
        std::cerr << "CollisionScene::isCollisionFree: joint size mismatch" << std::endl;
        return false;
    }
...
CollisionScene::loadMesh(const std::string& mesh_path) const {
    ...
    const aiScene* scene = importer.ReadFile(
        mesh_path,
        aiProcess_Triangulate | aiProcess_FlipWindingOrder | aiProcess_GenNormals);
...
void CollisionScene::buildAdjacencyList() {
    ...
    // This is a simplification; a proper implementation would extract joint connectivity from the robot model
    for (int i = 0; i < model_.numJoints(); ++i) {
        if (i > 0) {
...
void CollisionScene::updateCollisionTransforms(const std::vector<double>& q) const {
    ...
    for (size_t i = 0; i < link_objects_.size(); ++i) {
        for (size_t j = i + 1; j < link_objects_.size(); ++j) {
            ...
            coal::collide(link_objects_[i].collision_obj.get(),
                         link_objects_[j].collision_obj.get(),
                         request, result);
