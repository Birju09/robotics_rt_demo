...
RTExecutor::RTExecutor(OutputCallback on_tick)
    : on_tick_(on_tick) {
}
...
void RTExecutor::rtLoop() {
    ...
    if (ret != 0) {
        std::cerr << "Warning: Failed to set SCHED_FIFO. Are you running as root or with CAP_SYS_NICE?"
                  << std::endl;
        std::cerr << "        Falling back to SCHED_OTHER (non-realtime)" << std::endl;
    } else {
    ...
    uint64_t now_ns = static_cast<uint64_t>(now.tv_sec) * 1000000000UL + now.tv_nsec;
    ...
    while (running_.load(std::memory_order_acquire)) {
        ...
        if (ret != 0 && ret != EINTR) {
            std::cerr << "RTExecutor: clock_nanosleep failed with error " << ret << std::endl;
        }
