...
Planner::Planner(const RobotModel &model, const CollisionScene &collision,
                 TrajectoryCallback on_new_trajectory)
    : model_(model), collision_(collision), callback_(on_new_trajectory) {}
...
void Planner::requestPlan(const KDL::Frame &target_pose,
                          ...
                          const std::vector<double> &q_current) {
  {
    ...
    pending_request_ =
        PlanRequest{target_pose, trajectory_duration_s, q_current};
  }
...
void Planner::planningLoop() {
  while (running_.load()) {
    ...
    auto trajectory =
        std::make_shared<Trajectory>(fitCubicSpline(path, request->duration_s));

    std::cout << "Planner: trajectory ready, duration="
              << trajectory->duration_s << "s" << std::endl;
...
Planner::runOMPL(const std::vector<double> &q_start,
                 const std::vector<double> &q_goal, double time_limit_s) {
