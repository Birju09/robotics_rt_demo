...
bool RobotModel::fk(const std::vector<double> &q, KDL::Frame &out_frame) const {
  if (q.size() != static_cast<size_t>(kdl_chain_.getNrOfJoints())) {
    std::cerr << "FK: joint size mismatch. Expected "
              << kdl_chain_.getNrOfJoints() << ", got " << q.size()
              << std::endl;
    return false;
  }
...
bool RobotModel::ik(const KDL::Frame &target, const std::vector<double> &q_seed,
  ...
  if (q_seed.size() != static_cast<size_t>(kdl_chain_.getNrOfJoints())) {
    std::cerr << "IK: seed size mismatch. Expected "
              << kdl_chain_.getNrOfJoints() << ", got " << q_seed.size()
              << std::endl;
    return false;
  }
...
void RobotModel::jointLimits(std::vector<double> &lower,
...
void RobotModel::extractLinkMeshes(const urdf::ModelInterface &urdf_model) {
  for (const auto &[link_name, link] : urdf_model.links_) {
    ...
    double rx = pose.rotation.x, ry = pose.rotation.y, rz = pose.rotation.z,
           rw = pose.rotation.w;
    lm.visual_offset = KDL::Frame(KDL::Rotation::Quaternion(rx, ry, rz, rw),
                                  KDL::Vector(x, y, z));
